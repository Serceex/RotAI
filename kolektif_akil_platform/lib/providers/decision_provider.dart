...
class DecisionProvider with ChangeNotifier {
  ...
  Future<Decision> analyzeDecision(String question, {required String userId}) async {
    _isAnalyzing = true;
    notifyListeners();
    ...
    try {
      ...
      // Extract options from decision tree
      final optionA = decisionTree.rootNode.children.isNotEmpty
          ? decisionTree.rootNode.children[0].label
          : 'Seçenek A';
      final optionB = decisionTree.rootNode.children.length > 1
          ? decisionTree.rootNode.children[1].label
          : 'Seçenek B';
